...
function displayTime(maintime, mts) {
...
const mts = setInterval(() => {
	const getMT = new XMLHttpRequest();
	getMT.open("POST", "pr/processes/user/mainTimer.php", true);
	getMT.onload = function () {
		if (getMT.status === 200) {
			displayTime(getMT.responseText, mts);
		}
	};
	getMT.send();
}, 1000);
